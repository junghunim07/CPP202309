...
void checkExceptOutMap(User* user);
...
int main() {
	...
	while (1) { // 사용자에게 계속 입력받기 위해 무한 루프
		...
		if (user_input == "상") {
			...
			try {
				checkExceptOutMap(current_user);
...
// 맵을 벗어났는지 체크하고 벗어났으면 exception throw할 함수
void checkExceptOutMap(User* user) {
	if ((user->GetX() < 0 || user->GetX() > mapX) || (user->GetY() < 0 || user->GetY() > mapY)) {
		throw invalid_argument("맵을 벗어났습니다.");
		cout << "에러: " << endl;
	}
}
