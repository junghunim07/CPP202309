	...
	while (true) {
		...
		for (int i = 0; i < numCell; i++) {
			...
			if ((board[i][0] == board[i][1]) && (board[i][0] == board[i][2])) {
				if (board[i][0] != ' ') {
					cout << "가로에 모두 돌이 놓였습니다!: " << k % 2 + 1 << "번 유저(" << board[i][0] << ")의 승리입니다.";
			...
			// 세로가 같은 돌로 한줄로 다 놓였을 경우
				...
				if (board[0][i] != ' ') {
					cout << "세로에 모두 돌이 놓였습니다!: " << k % 2 + 1 << "번 유저(" << board[0][i] << ")의 승리입니다.";
					tmp = 1;		// 게임을 끝내기 위해서 tmp변수를 1로 초기화.
					break;
				}
			}
			...
			if (i == 0) {
				...
				if (board[i][0] == board[i + 1][1] && board[i][0] == board[i + 2][2]) {
					if (board[i][0] != ' ') {
						cout << "왼쪽 위에서 오른쪽 아래 대각선으로 모두 돌이 놓였습니다!: " << k % 2 + 1 << "번 유저(" << board[0][i] << ")의 승리입니다.";
						tmp = 1;	// 게임을 끝내기 위해서 tmp변수를 1로 초기화.
						break;
					}
				}
			...
			// 왼쪽 대각선
				...
				if (board[0][i] == board[1][i - 1] && board[0][i] == board[2][i - 2]) {
					if (board[0][i] != ' ') {
						cout << "오른쪽 위에서 왼쪽 아래 대각선으로 모두 돌이 놓였습니다!: " << k % 2 + 1 << "번 유저(" << board[0][i] << ")의 승리입니다.";
						tmp = 1;	// 게임을 끝내기 위해서 tmp변수를 1로 초기화.
						break;
					}
				}
